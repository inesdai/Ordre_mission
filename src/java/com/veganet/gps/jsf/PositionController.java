...
//          testSelectedNode.setRowKey("root");
    ...
    //edit command type to 1 
    public void sendCommandCheckGPS() {
        if (!vehiclesInMonitoring.isEmpty()) {
            String returnCommand = "";
            Device deviceCheck = vehiclesInMonitoring.get(0).getDevice();
            deviceCheck.setCommandtype((short) 1);
            deviceCheck.setReturncommand("");
            deviceFacade.edit(deviceCheck);
            ...
            for (int i = 0; i < 30; i++) {
                try {
                    ...
                    Thread.sleep(10000);
                    deviceCheck = deviceFacade.refreshDevice(deviceCheck);
                    if (deviceCheck.getCommandtype() == 10) {
                        returnCommand = deviceCheck.getReturncommand();
                        break;
                    }
            ...
            if (new String(returnCommand).equals("Heartbit")) {
                returnCommand = "Un Heartbit est reçu de la part du GPS.";
                requestContext.execute("alert('" + returnCommand + "');");
            } else if (new String(returnCommand).equals("") || returnCommand == null) {
                returnCommand = "Le GPS ne repend pas !";
                requestContext.execute("alert('" + returnCommand + "');");
            } else {
                
                requestContext.execute("if (window.confirm('Votre GPS est en déplacement avec une vitesse:" + returnCommand.split(",")[2] + " KM/H si vous cliquez sur OK un onglet vas s ouvrir pour montrer votre position')) { var win = window.open('"+"http://maps.google.com/maps?q=" + returnCommand.split(",")[0] + "," + returnCommand.split(",")[1]+"', '_blank'); win.focus(); };");
            }

            deviceCheck.setCommandtype((short) 0);
            deviceCheck.setReturncommand("");
            deviceFacade.edit(deviceCheck);
        }
    ...
    public void generateFeedBackJsonString(boolean interpolate) {
        ...
        try {
            ...
            if (positionsFeedBack.size() == 0) {
                ...
                nexPosition = new Position();

                precPosition = vehiclesInMonitoring.get(vehiclesInMonitoring.size() - 1).getDevice().getLastpositionid();

                precPosition = getFacade().findBy("Position.findByBeforePosition", "deviceid", vehiclesInMonitoring.get(vehiclesInMonitoring.size() - 1).getDevice(), "timecreate", from);
//                precPosition.setTimecreate(vehiclesInMonitoring.get(vehiclesInMonitoring.size() - 1).getDevice().getLastpositionid().getTimeCreateWitoutconvertion());

                Calendar calendar = GregorianCalendar.getInstance(); // creates a new calendar instance
                calendar.setTime(to);
                calendar.add(Calendar.HOUR_OF_DAY, +9);
                nexPosition.setTimecreate(calendar.getTime());
                calendar.setTime(from);
                calendar.add(Calendar.HOUR_OF_DAY, +9);
                precPosition.setTimecreate(calendar.getTime());
                nexPosition.setLatitude(36.00);
                nexPosition.setLongitude(10.00);
                nexPosition.setOther("null");
                nexPosition.setSpeed(0.0);
                nexPosition.setValid((short) 1);

                List<Position> emptyPositions = new ArrayList<>();
                emptyPositions.add(precPosition);
                emptyPositions.add(nexPosition);
                jsonStopPoints = mapper.writeValueAsString(emptyPositions);
                FacesContext.getCurrentInstance().getExternalContext()
                        .redirect("/gpstracker/faces/device/EmptyFeedBack.xhtml");
...
//        System.out.println(calendar.getTime());
    ...
    public Date addOneHour(Date date) {
        Calendar calendar = GregorianCalendar.getInstance(); // creates a new calendar instance
        calendar.setTime(date);
...
//        System.out.println(calendar.getTime());
    ...
    public void drawFeedBack() {
        try {
            ...
            System.out.println("from feedback " + ConvertDateToCst(from));
            System.out.println("to feedback " + ConvertDateToCst(to));
            infoWindowFeedback
    ...
    public void StopPoints() {
        ...
        System.out.println("from stop " + ConvertDateToCst(from));
        System.out.println("to stop " + ConvertDateToCst(to));
        if (positionsFeedBack.size() > 0) {
...
//     0.0001==11 Meter/0.00012==11 Meter
    ...
    public void overSpeedPositions() {
        ...
        System.out.println("from overspeed " + ConvertDateToCst(from));
        System.out.println("to overspeed " + ConvertDateToCst(to));
